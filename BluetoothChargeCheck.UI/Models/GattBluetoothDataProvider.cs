...
internal class GattBluetoothDataProvider
{
    ...
    public static async IAsyncEnumerable<IBluetoothDevice> FetchDevicesAsync()
    {
    ...
    /// <summary>
    /// Converts windows bluetooth LE device to <see cref="IBluetoothDevice"/> and tries to retrieve charge status from GATT server.
    /// </summary>
    private static async Task<IBluetoothDevice> ToBluetoothDevice(BluetoothLEDevice device) => new BluetoothDevice1()
    {
        Name = device.Name,
        Id = device.DeviceId,
